// src/screens/ConnectFriendsScreen.tsx 
...
export default function ConnectFriendsScreen() {
  ...
  // ← UPDATED: Enhanced handleConnect with new validation
  const handleConnect = async () => {
    ...
    try {
      ...
      if (!receiver?.id) {
        Alert.alert(
          "Oops! 😅",
          "We cannot find anyone with this referral code."
        );
        return;
      }
      ...
      // ← NEW: Check if request can be sent using new validation
      const canSend = await canSendRequest(currentUserId, receiver.id);
      if (!canSend.canSend) {
        Alert.alert("Cannot Send Request", canSend.reason || "Unknown error");
        return;
      }
      ...
      // ← NEW: Handle specific error messages from new validation
      if (error.message) {
  ...
  const handleApproveRequest = async (requestId: string) => {
    try {
      ...
      // ← New acceptRequest will handle opposing requests automatically
