...
// src/screens/MoodListScreen.tsx - UPDATED với Delete Friends
import React, { useEffect, useState } from "react";
import {
  FlatList,
  Text,
...
export default function MoodListScreen({ navigation }: any) {
  ...
  const { user, setUser } = useAuth(); // ← ADD: setUser for updating current user after friend deletion
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        {/* ← UPDATED: Enhanced Friends Selection Dropdown */}
        <View style={styles.pickerContainer}>
          <Text style={styles.pickerLabel}>Viewing:</Text>
          <View>
            <CustomModalPicker
              ...
              selectedId={selectedUserId}
              onValueChange={setSelectedUserId}
              currentUserId={user?.id} // ← ADD: Pass current user ID
              onFriendDeleted={handleFriendDeleted} // ← ADD: Pass callback
              enableDelete={true} // ← ADD: Enable delete functionality
            />
          </View>
        </View>
